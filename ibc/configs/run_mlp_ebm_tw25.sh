...
python3 ibc/ibc/train_eval.py -- \
  ...
  --add_time=True \
  --skip_eval=True \
  --saving_folder=awo_testin_bigbs_lr2_decayfast2 \
  --decay_steps=10 \
  --decay_rate=0.9 \
   --gin_bindings="train_eval.dataset_path='ibc/data/UR5_single/tw_data*.tfrecord'" \
  --gin_bindings="train_eval.batch_size=2048" \
  --gin_bindings="MLPEBM.width=1024" \
  --gin_bindings="MLPEBM.depth=8" \
  --gin_bindings="train_eval.learning_rate=5e-7" \
  $@
