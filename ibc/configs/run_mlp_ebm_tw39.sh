...
python3 ibc/ibc/train_eval.py -- \
  ...
  --add_time=True \
  --skip_eval=True \
  --saving_folder=awo_testin_bigbs_lr2_decayfast16 \
  --decay_steps=4 \
  --decay_rate=0.8 \
   --gin_bindings="train_eval.dataset_path='ibc/data/UR5_single/tw_data_0.tfrecord'" \
  --gin_bindings="train_eval.batch_size=8192" \
  --gin_bindings="MLPEBM.width=2048" \
